...
FileSystemManager.prototype.determineEnv = function() {
  if (_isBoolean(this.isDevelop)) return;
  this.isDevelop = this.env.getProcessProperty(ENV_TYPE) === ENV_TYPE_DEV;
};
...
FileSystemManager.prototype.getModule = function(key) {
  ...
  key = this.isDevelop ? key + CONFIG_DEV_POST : key;
